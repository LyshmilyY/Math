...
2. 设函数 $f(x)$ 在 $(-\infty,+\infty)$ 上二阶导数连续,$f(1)\leq 0$,$\lim\limits_{x\rightarrow \infty}\left[ f(x)-|x|\right]=0$, 证明:
...
2. 设 $f(x)$ 二阶可导, 且 $f(1)=6,f'(1)=0$, 且当 $x\geq 1, x^{2}f''(x)-3xf'(x)-5f(x)\geq 0$, 证明: 当 $x\geq 1$ 时,$f(x)\geq x^{5}+\dfrac{5}{x}$
\myspace{1}
\begin{solution}
...
2. 设函数 $f(x),g(x)$ 在 $[0,1]$ 上二阶可导,且 $f(1)>g(1),f(0)>g(0)$,$\displaystyle{\int_{0}^{1}f(x)dx=\int_{0}^{1}g(x)dx}$, 试证明: $\exists \xi\in(0,1),\ s.t.\ f''(\xi)>g''(\xi)$
\myspace{1}
\begin{solution}
...
2. 设 $f_{x}(x_{0},y_{0})$ 存在,$f_{y}(x_{0},y_{0})$ 在点 $(x_{0},y_{0})$ 处连续, 证明: $f(x,y)$ 在点 $(x_{0},y_{0})$ 处可微
\myspace{1}
\begin{solution}
...
3. 已知函数 $f(x,y)$ 在点 $(1,1)$ 处的梯度 $\mathbf{grad} f(1,1)=2i-j$,求函数 $f(x,y)$ 在该点沿曲线 $e^{x-1}+xy=2$ 在该点处切线方向(与 $y$ 轴正向夹角小于 $\frac{\pi}{2}$)的方向导数
\myspace{1}
\begin{solution}
