...
\begin{solution}
	...
	$f[\varphi(x)]=1-x\Rightarrow e^{\varphi^{2}(x)}=1-x$, 因此: $\varphi(x)=\sqrt{\ln (1-x)}, x\leq 0$
\end{solution}
\myspace{1}
...
\begin{example}[][Exam: 27.3.8]
	已知 $\lim\limits_{x\to 0}\dfrac{x-\arctan x}{x^{k}}=c$, 其中 $k,c$ 为常数, 且 $c\neq 0$,求 $k,c$
\end{example}

\begin{solution}

	\begin{align*}
		I = & \lim\limits_{x\to 0}\dfrac{x-\arctan x}{x^{k}}\\
...
\begin{example}[][Exam: 27.3.9]
	若 $\lim\limits_{x\to 0}\left(\dfrac{\sin x^{3}}{x^{4}}-\dfrac{f(x)}{x^{3}}\right)=2$,  $x\to 0$ 时,$f(x)$ 是 $x$ 的:
\begin{itemize}
	\item A. 等价无穷小
	\item B. 同阶但非等价无穷小
	\item C. 高阶无穷小
...
\begin{solution}
	...
	$$\lim\limits_{x \to 0}\left(\dfrac{\sin x^{3}}{x^{4}}- \dfrac{f(x)}{x^{3}}\right)x^{2} = 0$$
	$$\lim\limits_{x \to 0}\dfrac{f(x)}{x} = \lim\limits_{x \to 0}\dfrac{\sin x^{3}}{x^{2}} = 0$$
