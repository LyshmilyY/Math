...
1. 求极限 $\lim\limits_{x\to +\infty}\left[ \dfrac{\ln(x+\sqrt{x^2+1})}{\ln(x+\sqrt{x^2-1})}\right]^{x^2\ln x} $
\myspace{1}
\begin{solution}
...
1. 设$f(x)$连续,$\lim\limits_{x\to 0}\dfrac{f(x)}{x}=1$,求极限$\lim\limits_{x\to 0 }\left[ 1+\int_{0}^{x}tf(x^2-t^2)dt\right]^{\frac{1}{(\tan x-x)\ln(1+x)}} $
\myspace{1}
\begin{solution}
...
2. 设$f(x)$是$(-\infty,+\infty)$上连续的周期为$1$的周期函数且满足$0\leq f(x)\leq 1$与$\int_{0}^{1}f(x)dx=1$,证明: $x\in[0,13]$时,$\int_{0}^{\sqrt{x}}f(t)dt+\int_{0}^{\sqrt{x+27}}f(t)dt+\int_{0}^{\sqrt{13-x}}f(t)dt\leq 11$
\myspace{1}
\begin{solution}
...
1. 设$D=\{(x,y)|0\leq x\leq 2,0\leq y\leq 2\}$,$f(x,y)$在$D$上连续,$\iint\limits_{D}f(x,y)dxdy=0$,$\iint\limits_{D}xyf(x,y)dxdy=1$,证明: $\exists (\xi,\eta)\in D,\ s.t. |f'(\xi,\eta)|\geq \dfrac{2}{4\ln 2+3}$
\myspace{1}
\begin{solution}
...
2. 求极限: $\lim\limits_{x\to+\infty}(x^{\frac{1}{x}}-1)^{\frac{1}{\ln x}}$
\myspace{1}
\begin{solution}
...
1. 已知$a_{n}=\sum\limits_{k=1}^{n}\dfrac{1}{k^2}$,$b_{n}=\sum\limits_{k=1}^{n}\dfrac{1}{(2k-1)^2}$,且$\lim\limits_{n\to +\infty}a_{n}=\dfrac{\pi^2}{6}$

(1). 证明: $a_{2n}=\dfrac{1}{4}a_{n}+b_{n},\ (n=1,2,\cdots)$

(2). 求$\lim\limits_{n\to+\infty}n\left( \dfrac{b_{n}}{a_{n}}-\dfrac{3}{4}\right) $
\myspace{1}
\begin{solution}
...
2. 若$\lim\limits_{x\to 0}\dfrac{\cos(xe^x)-e^{-\frac{x^2}{2}e^{2x}}}{x^{\alpha}}=\beta\neq0$,求$\alpha,\beta$
\myspace{1}
\begin{solution}
...
2. 若$\lim\limits_{x\to 0}\dfrac{\cos 2x-\cos x\sqrt{\cos 2x}}{x^{\alpha}}=\beta\neq0$,求$\alpha,\beta$
\myspace{1}
\begin{solution}
...
2. 如果$\lim\limits_{x\to 0}\dfrac{ax^2+bx+1-e^{x^2-2x}}{x^2}=2$,求$a,b$
\myspace{1}
\begin{solution}
...
1. 求极限$\lim\limits_{x\to 0}\dfrac{n!x^n-\sin x\sin 2x\cdots\sin nx}{x^{n+2}}$
\myspace{1}
\begin{solution}
...
2. 若$\lim\limits_{x\to 0}\left(\dfrac{\ln(x+\sqrt{x^2+1})+ax^2+bx^3}{x} \right)^{\frac{1}{x^2}}=e^2$,求$a,b$
\myspace{1}
\begin{solution}
...
2. 已知常数$a>0,\ bc\neq 0$,使得$\lim\limits_{x\to+\infty}[x^a\ln(1+\frac{x}{b})-x]=c$,求$a,b,c$
\myspace{1}
\begin{solution}
...
1. $s>0$,证明: $\dfrac{n^{s+1}}{s+1}<1^s+2^s+\cdots+n^s<\dfrac{(n+1)^{s+1}}{s+1}$
\myspace{1}
\begin{figure}[H]
	\centering
...
2. 求极限$\lim\limits_{n\to +\infty}\dfrac{1^{p+1}+2^{p+1}+\cdots+n^{p+1}}{n(1^p+2^p+\cdots+n^p)}(p>0)$
\myspace{1}
\begin{solution}
...
2. 设$3$阶实对称矩阵$A$特征值为$\lambda_{1}=-1,\ \lambda_{2}=\lambda_{3}=1$,对应于$\lambda_{1}$的特征向量为$\xi_{1}=[0,1,1]^{T}$,求$A$.
\myspace{1}
\begin{solution}
...
1. 求曲线$y=\sqrt{x^4-3x^3+4}$在$x\to +\infty$方向的渐进二次曲线.
\myspace{1}
\begin{solution}
	\begin{eqnarray*}
...
2. 设$\alpha_{1},\alpha_{2},\cdots,\alpha_{n}$是$n$个$n$维列向量,证明: $\alpha_{1},\alpha_{2},\cdots,\alpha_{n}$线性无关的充要条件为: $$\left| \begin{matrix}
...
2. 求级数$\sum\limits_{n=1}^{+\infty}\dfrac{(-1)^n}{(2n-1)(2n+1)}$的和.
\myspace{1}
\begin{solution}
...
2. 求$\sum\limits_{n=0}^{+\infty}\dfrac{(-1)^n(n^2-n+1)}{2^n}$的和.
\myspace{1}
\begin{solution}
...
2. 设$f(x)$连续,且$\lim\limits_{x\to 0^{+}}\dfrac{f(x)}{x}=1$,$\alpha(x)=\int_{0}^{\sqrt{x}}\dfrac{\ln(1+t^4)}{f(t)}dt$,$\beta(x)=\int_{0}^{\sin x}\dfrac{\sqrt{1+t^3}-1}{f(t)}dt$,则当$x\to 0^{+}$时,$\alpha(x)$是$\beta(x)$的: 
\begin{itemize}
	\item A. 等价无穷小
	\item B. 同阶但非等价无穷小
	\item C. 高阶无穷小
...
2. 当$x\to 0$时,$2\arctan x-\ln\dfrac{1+x}{1-x}$是$x$的$n$阶无穷小,求$n$.
\myspace{1}
\begin{solution}
...
\begin{itemize}
	...
	\item \hl{C}. $\lim\limits_{(x,y)\to (0,0)}f(x,y)=0$
	\item \hl{D}. $\lim\limits_{x\to 0}\lim\limits_{y\to 0}f(x,y)=0$
\end{itemize}
\myspace{1}
\begin{solution}
...
2. 求$\sum\limits_{n=0}^{+\infty}\dfrac{(2n)!!}{(2n+1)!!}\dfrac{x^{2n+2}}{n+1}$的和函数$S(x)$
\myspace{1}
\begin{solution}
...
1. 当$x\to 0^{+}$时,$(1+x)^{\frac{1}{x}}-(e+ax+bx^2)$是比$x^2$高阶的无穷小,求$a,b$
\myspace{1}
\begin{solution}
...
2. 求极限$\lim\limits_{x\to 0}\dfrac{e^{(1+x)^{\frac{1}{x}}}-(1+x)^{\frac{e}{x}}}{x^2}$
\myspace{1}
\begin{solution}
...
2. 设$f(x)$在$[-1,1]$二阶导数连续,证明: $\exists \xi\in[-1,1],\ s.t. \int_{-1}^{1}xf(x)dx=\dfrac{2f'(\xi)+\xi f''(\xi)}{3}$
\myspace{1}
\begin{solution}
...
1.函数$f(x)=\dfrac{(x+1)|x-1|}{e^{\frac{1}{x-2}}\ln|x|}$的可去间断点个数.
\myspace{1}
\begin{solution}
...
2. 设$f(x)$和$\varphi(x)$在$(-\infty,+\infty)$内有定义,$f(x)$为连续函数,$\varphi(x)$有间断点,下列命题正确的是: 
\begin{itemize}
...
1. 设$\lambda_{1},\lambda_{2},\cdots,\lambda_{m}$是$n$阶矩阵$A$的$m$个两两互异的特征值$(m\leq n)$,对应的特征向量分别为$\xi_{1},\xi_{2},\cdots,\xi_{m}$,令$\eta=\sum\limits_{k=1}^{m}\xi_{k}$,证明: $$\eta,A\eta,A^2\eta,\cdots,A^{m-1}\eta\text{线性无关}$$
\myspace{1}
\begin{solution}
...
2. 设$f(x)=\lim\limits_{n\to+\infty}\dfrac{x^{n+2}}{\sqrt{2^{2n}+x^{2n}}}$,则$f(x)$在其定义域内: 
\begin{itemize}
	\item A. 连续
	\item B. 有$1$个可去间断点
...
2. 设$f(x)=\lim\limits_{n\to +\infty}\dfrac{x^{n+2}-x^{-n}}{x^n+x^{-n}}$,则函数$f(x)$: 
\begin{itemize}
	\item A. 仅有$1$个间断点
...
2. 设$\alpha, \beta$均为$3$维单位列向量,$\alpha^{T}\beta=\dfrac{1}{2}$,$A=\alpha\beta^{T}+\beta\alpha^{T}$,求二次型$x^{T}Ax$在正交变换下的标准型.
\myspace{1}
\begin{solution}
...
2. 求不定积分$\int\dfrac{x^2(\sqrt{1+x^2}-x)}{\sqrt{1+x^2}(\sqrt{1+x^2}+1)}e^{x}dx$
\myspace{1}
\begin{solution}
...
2. $A=(a_{ij})_{3\times 3}$是实对称矩阵,$A$的每行元素之和为$0$,设$2,3$是$A$的非零特征值,求$a_{11}$对应的代数余子式.
\myspace{1}
\begin{solution}
...
2. 设$f(x)$在$[a,b]$连续,在$(a,b)$内可导,证明: $$\text{存在互不相同的}\xi_{1},\xi_{2},\cdots,\xi_{n}\in(a,b),\ s.t. \ f'(\xi_{1})f'(\xi_{2})\cdots f'(\xi_{n})=\left[ \dfrac{f(b)-f(a)}{b-a}\right]^{n}$$
\myspace{1}
\begin{solution}
...
1. 已知$f(x)$在$x=0$的某邻域内有定义,且$\lim\limits_{x\to 0}\varphi(x)=0$,则$\lim\limits_{x\to 0}\dfrac{f[\varphi(x)]-f(0)}{\varphi(x)}$存在和$f(x)$在$x=0$处可导之间关系
\myspace{1}
\begin{solution}
...
2. 设存在连续函数$f(x)$满足$f(x)=1+\lambda\int_{x}^{1}f(y)f(y-x)dy$,证明: $\lambda\leq \dfrac{1}{2}$
\myspace{1}
\begin{solution}
