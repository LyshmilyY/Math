...
2. 求极限$\lim\limits_{x\rightarrow 0}\int_{0}^{x}(\dfrac{\arctan t}{t})^{\dfrac{1}{\int_{0}^{t}\ln(1+u)du}}\cot xdt$
\myspace{1}
\begin{solution}
...
3. 设$D=\{(x,y)|0\leq x\leq 2,0\leq y\leq 2\}$,$f(x,y)$在$D$上连续,$\iint\limits_{D}f(x,y)dxdy=0$,$\iint\limits_{D}xyf(x,y)dxdy=1$,证明: $\exists (\xi,\eta)\in D,\ s.t. |f'(\xi,\eta)|\geq \dfrac{2}{4\ln 2+3}$
\myspace{1}
\begin{solution}
...
(2). 求$\lim\limits_{n\rightarrow+\infty}n\left( \dfrac{b_{n}}{a_{n}}-\dfrac{3}{4}\right) $
\myspace{1}
\begin{solution}
...
2. 求$\int_{0}^{+\infty}\dfrac{\ln x}{x^2+a^2}dx\ (a>0)$
\myspace{1}
\begin{solution}
...
2. 设$f(x)=\dfrac{1}{1+3x+9x^2}$,求$f^{(100)}(0)$
\myspace{1}
\begin{solution}
...
2. 求极限$\lim\limits_{x\rightarrow 0}\dfrac{n!x^n-\sin x\sin 2x\cdots\sin nx}{x^{n+2}}$
\myspace{1}
\begin{solution}
...
1.若$\lim\limits_{x\rightarrow 0}\left(\dfrac{\ln(x+\sqrt{x^2+1})+ax^2+bx^3}{x} \right)^{\frac{1}{x^2}}=e^2$,求$a,b$
\myspace{1}
\begin{solution}
...
1. 设$A$为$n$阶反对称矩阵,证明: 对于任意向量$x$,均有$x^{T}(A+E)x\geq 0$.
\myspace{1}
\begin{solution}
...
\begin{figure}[H]
	...
	\subfigure[定积分定义示意图$\alpha$]{
		\includegraphics[width=7.5cm,height=5cm]{"figure/Question/定积分定义-I.jpg"}}
	\subfigure[定积分定义示意图$\beta$]{
		\includegraphics[width=7.5cm,height=5cm]{"figure/Question/定积分定义-II.jpg"}}
	\caption{积分和求和关系图}
\end{figure} 
\begin{solution}
...
3.设$A$是任意的$m\times n$矩阵,证明: $\text{方程组}A^{T}Ax=A^{T}b$一定有解.
\myspace{1}
\begin{solution}
...
2. 求$f(x)=\sqrt{4x^2+x}\ln(2+\dfrac{1}{x})$的斜渐近线.
\myspace{1}
\begin{solution}
...
1. 设$\alpha_{1},\alpha_{2},\cdots,\alpha_{n}$是$n$个$n$维列向量,证明: $\alpha_{1},\alpha_{2},\cdots,\alpha_{n}$线性无关的充要条件为: $$\left| \begin{matrix}
...
2. 当$x\rightarrow 0^{+}$时,下列无穷小量中最高阶的是: 
\begin{itemize}
...
1. 求级数$\sum\limits_{n=1}^{+\infty}\dfrac{(-1)^n}{(2n-1)(2n+1)}$的和.
\myspace{1}
\begin{solution}
...
3. 求$\sum\limits_{n=0}^{+\infty}\dfrac{(-1)^n(n^2-n+1)}{2^n}$的和.
\myspace{1}
\begin{solution}
...
2. 设$f(x)$连续,且$\lim\limits_{x\rightarrow 0^{+}}\dfrac{f(x)}{x}=1$,$\alpha(x)=\int_{0}^{\sqrt{x}}\dfrac{\ln(1+t^4)}{f(t)}dt$,$\beta(x)=\int_{0}^{\sin x}\dfrac{\sqrt{1+t^3}-1}{f(t)}dt$,则当$x\rightarrow 0^{+}$时,$\alpha(x)$是$\beta(x)$的: 
\begin{itemize}
	\item A. 等价无穷小
	\item B. 同阶但非等价无穷小
	\item C. 高阶无穷小
...
3. 求$\sum\limits_{n=0}^{+\infty}\dfrac{(2n)!!}{(2n+1)!!}\dfrac{x^{2n+2}}{n+1}$的和函数$S(x)$
\myspace{1}
\begin{solution}
...
1. 设$f(x)$和$\varphi(x)$在$(-\infty,+\infty)$内有定义,$f(x)$为连续函数,$\varphi(x)$有间断点,下列命题正确的是: 
\begin{itemize}
...
2. 设$\lambda_{1},\lambda_{2},\cdots,\lambda_{m}$是$n$阶矩阵$A$的$m$个两两互异的特征值$(m\leq n)$,对应的特征向量分别为$\xi_{1},\xi_{2},\cdots,\xi_{m}$,令$\eta=\sum\limits_{k=1}^{m}\xi_{k}$,证明: $$\eta,A\eta,A^2\eta,\cdots,A^{m-1}\eta\text{线性无关}$$
\myspace{1}
\begin{solution}
...
1. 设$f(x)=\lim\limits_{n\rightarrow+\infty}\dfrac{x^{n+2}}{\sqrt{2^{2n}+x^{2n}}}$,则$f(x)$在其定义域内: 
\begin{itemize}
	\item A. 连续
	\item B. 有$1$个可去间断点
...
2. $\int_{0}^{1}\dfrac{x^b-x^a}{\ln x}dx$
\myspace{1}
\begin{solution}
...
2. $I_{n}=\int_{1}^{+\infty}\dfrac{\sqrt{t-1}}{t^n}dt\ (n\geq 2)$,求$\lim\limits_{n\rightarrow+\infty}\dfrac{I_{n+1}}{I_{n}}$
\myspace{1}
\begin{solution}
