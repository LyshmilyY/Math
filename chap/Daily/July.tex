...
2. 求极限: $\lim\limits_{x\rightarrow+\infty}(x^{\frac{1}{x}}-1)^{\frac{1}{\ln x}}$
\myspace{1}
\begin{solution}
...
1. 求极限$\lim\limits_{x\rightarrow 0}\dfrac{n!x^n-\sin x\sin 2x\cdots\sin nx}{x^{n+2}}$
\myspace{1}
\begin{solution}
...
2. 当$x\rightarrow 0$时,下列无穷小量最高阶的是: 
\begin{itemize}
	\item A. $(1+x)^{x^2}-1$
	\item B. $e^{x^4-2x}-1$
...
2. 当$x\rightarrow 0$时,$2\arctan x-\ln\dfrac{1+x}{1-x}$是$x$的$n$阶无穷小,求$n$.
\myspace{1}
\begin{solution}
...
2. 设$f(x)=\lim\limits_{n\rightarrow+\infty}\dfrac{x^{n+2}}{\sqrt{2^{2n}+x^{2n}}}$,则$f(x)$在其定义域内: 
\begin{itemize}
	\item A. 连续
	\item B. 有$1$个可去间断点
