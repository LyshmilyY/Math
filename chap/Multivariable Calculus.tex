...
设 $D$ 和 $D^{'}$ 是平面上两个(有界)区域, $D$ 到 $D^{'}$ 的对应 $\varphi :(u,v)\rightarrow(x,y)$ (这里 $x=x(u,v),y=y(u,v)$ 连续可微),
称为变量替换,要求 $\varphi$ 在一个面积为 $0$ 的集合外是 $1\sim 1$
...
1. $\displaystyle{\int_{0}^{+\infty}e^{-x^{2}}dx}$
\begin{anymark}[注]
...
\begin{proof}
	...
	$S_{n}$ 有界, 单调有界准则: $\lim\limits_{n\rightarrow+\infty}S_{n}$ 存在, 原级数收敛
\end{proof}
\myspace{1}
...
\begin{theorem}[比值判别法]
	正项级数 $\sum\limits_{n=1}^{+\infty}u_{n}$, 且 $\lim\limits_{n\rightarrow+\infty}\dfrac{u_{n+1}}{u_{n}}=\rho$
	\myspace{1}
